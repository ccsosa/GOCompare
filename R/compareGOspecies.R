...
compareGOspecies <- function(df1,df2,GOterm_field,species1,species2){
  ...
  pb <- txtProgressBar(min = 0, max = length(comb_feat_3), style = 3)
...
#tapply(x$shared_GO_list$feature,x$shared_GO_list$feature,length)
#tapply(x$unique_GO_list$feature,x$unique_GO_list$feature,length)
